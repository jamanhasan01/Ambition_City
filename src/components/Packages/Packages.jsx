  ...
  return (
    <Container>
      <div className='flex flex-col gap-5 py-[50px]'>
        <h2 className='h2 text-center'>Tour departures with best prices and services</h2>
        <div className='relative flex items-center justify-center'>
          ...
          >
            <li className={`whitespace-nowrap ${toggle === 1 ? 'text-mainClr border-b-2 border-mainClr' : ''}`} onClick={() => navChange(1)}>FEATURED PACKAGES</li>
            <li className={`whitespace-nowrap ${toggle === 2 ? 'text-mainClr border-b-2 border-mainClr' : ''}`} onClick={() => navChange(2)}>SPECIAL PACKAGES</li>
            <li className={`whitespace-nowrap ${toggle === 3 ? 'text-mainClr border-b-2 border-mainClr' : ''}`} onClick={() => navChange(3)}>LATEST PACKAGES</li>
            <li className={`whitespace-nowrap ${toggle === 4 ? 'text-mainClr border-b-2 border-mainClr' : ''}`} onClick={() => navChange(4)}>BESTSELLER PACKAGES</li>
          </nav>
          <button
          ...
          {slice.map((item) => {
            ...
            return (
              <div key={item.id} className='flex flex-col w-full max-w-[400px] p-5 gap-3 relative'>
                <span className='absolute right-8 top-8 cursor-pointer bg-white rounded-full px-[3px] py-[8x]' onClick={() => favoriteFunc(item.id)}>
                  {isFavorite ? <FavoriteIcon className='!text-lg text-red-500'/> : <FavoriteBorderIcon className='!text-lg'/>}
                </span>
                ...
                <div className='flex justify-between items-center'>
                  ...
                  <span className='!text-mainClr font-medium flex gap-1'>
                    <StarIcon className='!text-xl mb-1' /><span>{item.start}</span>
                  </span>
                </div>
